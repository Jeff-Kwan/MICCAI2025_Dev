...
from utils.explore_dataset import get_transforms, get_data_files
from model.ConvSeg2 import ConvSeg2
from utils.explore_trainer import DDPTrainer
...
def main_worker(rank: int,
    ...
    try:
        ...
        # Datasets
        train_tf_gt, val_tf = get_transforms(
            train_params['shape'],
        ...
        scheduler = CosineAnnealingLR(optimizer, T_max=train_params['epochs'], eta_min=train_params['min_lr'])
...
if __name__ == "__main__":
    ...
    model_params = json.load(open(f"configs/explore/convseg2.json"))
    train_params = json.load(open(f"configs/explore/train.json"))

    output_dir = f"Explore"
    comments = get_comments(output_dir, train_params)


    print(f"Starting exploration training...")
