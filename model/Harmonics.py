...
class HyperEdgeAttention(nn.Module):
    ...
    def _compute_edges(self, tokens: torch.Tensor) -> torch.Tensor:
        w = self.hyperedge(tokens)
        w = F.relu(w) * F.softmax(w, dim=-1) * (self.edges/S1/S2/S3)
        w = self.dropout(w)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        tokens = self.in_norm(tokens)                                    # (B, N, C)

        if self.training and x.requires_grad:
            z = checkpoint.checkpoint(self._compute_edges, tokens, use_reentrant=False)
        else:
            z = self._compute_edges(tokens)                              # (B, E, C)
            
        y = self.mha(tokens, z, z, need_weights=False)[0]                # (B, N, C)
